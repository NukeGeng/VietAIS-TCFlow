...
| Self-host bundle configuration and image builds | CONFIRMED (local build) | `deploy/self-host/docker-compose.yml` renders successfully with `.env.example`; Compose builds the API and Vue/Nginx images, and `nginx -t` passes with the Compose service name resolved |
