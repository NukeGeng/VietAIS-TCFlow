...
| 11. Self-host/LAN operation is maintainable | INFERRED / PROPOSED | Aspire startup, migrations, health checks, configuration validation, and repeatable README commands are confirmed. The versioned Docker Compose bundle, secret-safe environment template, health-gated services, and backup/migration upgrade runbook are now locally config/build-verified; production runtime, TLS, registry, and operator rollout remain external verification. |
