...
internal sealed class MartenAnalysisAccumulator(string analyzerName)
{
    ...
    public string AddEvidence(
        ...
        string? symbol = null)
    {
        var line = MartenTextParsing.LineNumber(file.Content, index);
    ...
    {
        var id = StableIdentity.Create("artifact", "marten", kind.ToString(), file.RelativePath, name);
        ...
        string evidenceId)
    {
        ...
        _dependencies[id] = new Dependency(
            id,
            sourceArtifactId,
            target,
            kind,
