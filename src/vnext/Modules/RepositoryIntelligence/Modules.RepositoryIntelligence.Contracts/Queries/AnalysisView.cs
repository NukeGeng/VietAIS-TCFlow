...
public sealed record SourceImpactView(string ImpactKey, string ChangeKey, string AffectedArtifactKey, string Severity, string Reason, decimal Confidence);
public sealed record AnalysisView(Guid Id, Guid ProjectId, string RepositoryId, string CommitSha, bool Completed, long Version, IReadOnlyList<SourceArtifactView> Artifacts, IReadOnlyList<SourceChangeView> Changes, IReadOnlyList<EvidenceView> Evidence, IReadOnlyList<SourceImpactView> Impacts, DateTimeOffset LastChangedAtUtc);
