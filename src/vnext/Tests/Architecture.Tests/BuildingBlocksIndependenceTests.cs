...
public class BuildingBlocksIndependenceTests
{
    ...
    private static readonly string SourceRoot = Directory.Exists(Path.Combine(SolutionRoot, "BuildingBlocks"))
        ? SolutionRoot
        : Path.Combine(SolutionRoot, "src", "vnext");
    ...
    {
        string buildingBlocksRoot = Path.Combine(SourceRoot, "BuildingBlocks");
    ...
    public void Core_BuildingBlock_Should_Be_Dependency_Free()
    {
        ...
        string coreProjectPath = Path.Combine(SourceRoot, "BuildingBlocks", "Core", "Core.csproj");
