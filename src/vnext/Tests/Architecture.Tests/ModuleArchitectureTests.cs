...
public class ModuleArchitectureTests
{
    ...
    public void Modules_Should_Not_Depend_On_Other_Modules()
    {
        ...
        string modulesRoot = ResolveSourcePath(solutionRoot, "Modules");
    ...
    private static string ResolveSourcePath(string solutionRoot, string relativePath)
    {
        var currentLayoutPath = Path.Combine(solutionRoot, "src", relativePath);
        return Directory.Exists(currentLayoutPath)
            ? currentLayoutPath
            : Path.Combine(solutionRoot, "src", "vnext", relativePath);
    }
}
